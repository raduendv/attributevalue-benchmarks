...
func (ss *User) UnmarshalDynamoDBAttributeValue(in types.AttributeValue) error {
	...
	if raw, ok := m["roles"]; ok && raw != nil {
		ss.Roles = func() []string {
			av, ok := raw.(*types.AttributeValueMemberL)
			if !ok {
				return nil
			}
			out := make([]string, len(av.Value))
			for i, item := range av.Value {
				if mv, ok := item.(*types.AttributeValueMemberS); ok {
					out[i] = mv.Value
				}
			}
			return out
	...
	if raw, ok := m["notification_channels"]; ok && raw != nil {
		ss.NotificationChannels = func() []string {
			av, ok := raw.(*types.AttributeValueMemberL)
			if !ok {
				return nil
			}
	...
	if raw, ok := m["settings"]; ok && raw != nil {
		ss.Settings = func() map[string]string {
			av, ok := raw.(*types.AttributeValueMemberM)
			if !ok {
				return nil
			}
			out := make(map[string]string, len(av.Value))
	...
	if raw, ok := m["feature_flags"]; ok && raw != nil {
		ss.FeatureFlags = func() map[string]bool {
			av, ok := raw.(*types.AttributeValueMemberM)
			if !ok {
				return nil
			}
			out := make(map[string]bool, len(av.Value))
			for k, item := range av.Value {
				if mv, ok := item.(*types.AttributeValueMemberBOOL); ok {
					out[k] = mv.Value
				}
			}
			return out
	...
	if raw, ok := m["quota_by_region"]; ok && raw != nil {
		ss.QuotaByRegion = func() map[string]int {
			av, ok := raw.(*types.AttributeValueMemberM)
			if !ok {
				return nil
			}
			out := make(map[string]int, len(av.Value))
			for k, item := range av.Value {
				if mv, ok := item.(*types.AttributeValueMemberN); ok {
					out[k] = func() int { v, _ := strconv.ParseInt(mv.Value, 10, strconv.IntSize); return int(v) }()
				}
			}
			return out
		}()
...
func (ss *User) MarshalDynamoDBAttributeValue() (types.AttributeValue, error) {
	...
	out["roles"] = &types.AttributeValueMemberL{Value: func() []types.AttributeValue {
		if ss.Roles == nil {
			return nil
		}
		out := make([]types.AttributeValue, 0, len(ss.Roles))
		for _, v := range ss.Roles {
			out = append(out, &types.AttributeValueMemberS{Value: v})
		}
		return out
	}()}
	out["login_ips"] = &types.AttributeValueMemberL{Value: func() []types.AttributeValue {
		if ss.LoginIPs == nil {
			return nil
		}
		out := make([]types.AttributeValue, 0, len(ss.LoginIPs))
		for _, v := range ss.LoginIPs {
			out = append(out, &types.AttributeValueMemberS{Value: v})
		}
		return out
	}()}
	out["notification_channels"] = &types.AttributeValueMemberL{Value: func() []types.AttributeValue {
		if ss.NotificationChannels == nil {
			return nil
		}
		out := make([]types.AttributeValue, 0, len(ss.NotificationChannels))
		for _, v := range ss.NotificationChannels {
			out = append(out, &types.AttributeValueMemberS{Value: v})
		}
		return out
	}()}
	out["settings"] = &types.AttributeValueMemberM{Value: func() map[string]types.AttributeValue {
		if ss.Settings == nil {
			return nil
		}
		out := make(map[string]types.AttributeValue, len(ss.Settings))
		for k, v := range ss.Settings {
			out[k] = &types.AttributeValueMemberS{Value: v}
		}
		return out
	}()}
	out["feature_flags"] = &types.AttributeValueMemberM{Value: func() map[string]types.AttributeValue {
		if ss.FeatureFlags == nil {
			return nil
		}
		out := make(map[string]types.AttributeValue, len(ss.FeatureFlags))
		for k, v := range ss.FeatureFlags {
			out[k] = &types.AttributeValueMemberBOOL{Value: v}
		}
		return out
	}()}
	out["quota_by_region"] = &types.AttributeValueMemberM{Value: func() map[string]types.AttributeValue {
		if ss.QuotaByRegion == nil {
			return nil
		}
		out := make(map[string]types.AttributeValue, len(ss.QuotaByRegion))
		for k, v := range ss.QuotaByRegion {
			out[k] = &types.AttributeValueMemberN{Value: strconv.FormatInt(int64(v), 10)}
		}
		return out
	}()}
